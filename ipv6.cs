  ...
  public static bool IsValidIPv6(string ipv6Address)
  {
    ...
    if (segments.Count != 0)
    {
      foreach (Match segment in segments)
      {
        ...
        {
          Console.WriteLine("Invalid IPv6 format: A segment can only have a max of four hex digits.");
          return false;           
        }
      }
    }
    ...
    else
    {
      ...
      if (segments.Count > 6)
      {
        Console.WriteLine("Invalid IPv6 format: IPv6 Address with double-colon were not used in proper format.");
        return false;         
      }
    }
